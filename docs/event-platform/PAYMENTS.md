...
| The Stripe fee on the Results page | `recordStripeFee` (`_shared/order-state.ts`) reads `latest_charge.balance_transaction.fee` on the account that took the money, after the webhook confirms and again from `stripe-reconcile` for any paid order in the last 30 days still without one. The simulator's fee is an assumed rate. A test-mode charge on each kind of account, compared with the fee shown in that account's Stripe dashboard, proves it. A fee settled in a currency other than the order's is shown as not yet known rather than netted off. |
