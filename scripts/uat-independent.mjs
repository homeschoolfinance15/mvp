...
try {
  ...
  await adminPage.goto(`${site}/manage/events/${ev.id}`)
  const namedHost = await user('named-host', 'connector')
  const namedConnector = await db('connectors', undefined, 'POST', { profile_id: namedHost.id, invite_status: 'active', invite_capacity: 1, can_create_events: false })
  if (!namedConnector.ok) throw Error(JSON.stringify(namedConnector.data))
  const namedRow = await db('event_hosts', admin.token, 'POST', { event_id: ev.id, profile_id: namedHost.id })
  if (!namedRow.ok) throw Error(JSON.stringify(namedRow.data))
  await adminPage.reload()
  const management = adminPage.getByRole('checkbox', { name: 'Allow UAT named-host to manage this event', exact: true })
  await management.waitFor()
  check('ORG-05 browser shows newly named host without management', !(await management.isChecked()))
  const grantReply = adminPage.waitForResponse(r => r.url().includes('/rpc/set_event_host_management') && r.request().method() === 'POST')
  await management.click()
  const granted = await grantReply
  await adminPage.getByText('Event manager', { exact: true }).waitFor()
  check('ORG-05 browser saves explicit management assignment', granted.ok() && (await rpc('hosts_event', namedHost.token, { p_event: ev.id })).data === true)
  const revokeReply = adminPage.waitForResponse(r => r.url().includes('/rpc/set_event_host_management') && r.request().method() === 'POST')
  await management.click()
  const revoked = await revokeReply
  await adminPage.getByText('Named host only', { exact: true }).waitFor()
  check('ORG-05 browser revokes management while retaining hosting credit', revoked.ok() && (await rpc('hosts_event', namedHost.token, { p_event: ev.id })).data === false)
